...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ParadiseCoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zPRDsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zPRD denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zPRD denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zPRD denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zPRD denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zPRD denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zPRD denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zPRD denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zPRD denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zPRD denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in ParadiseCoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in ParadiseCoin/kb\n"
    ...
    Object zParadiseCoinObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zParadiseCoinObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zParadiseCoinObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zPRDsupply", zParadiseCoinObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"ParadiseCoinaddress\"\n"
            "\nReturn information about the given ParadiseCoin address.\n"
            "\nArguments:\n"
            "1. \"ParadiseCoinaddress\"     (string, required) The ParadiseCoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ParadiseCoinaddress\", (string) The ParadiseCoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ParadiseCoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are ParadiseCoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) ParadiseCoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"ParadiseCoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"ParadiseCoinaddress\"  (string, required) The ParadiseCoin address to use for the signature.\n"
