...
#include "zParadiseCoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPRD ought to be enough for anybody." - Bill Gates, 2017
    ui->zPRDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPRDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPRD->setEnabled(false);
        ui->pushButtonMintzPRD->setToolTip(tr("zPRD is currently disabled due to maintenance."));

        ui->pushButtonSpendzPRD->setEnabled(false);
        ui->pushButtonSpendzPRD->setToolTip(tr("zPRD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPRDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPRD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPRD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPRD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPRD in ") +
...
void PrivacyDialog::on_pushButtonSpendzPRD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPRD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPRD
        sendzPRD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPRD
    sendzPRD();
}

void PrivacyDialog::on_pushButtonZParadiseCoinControl_clicked()
{
    ZParadiseCoinControlDialog* zParadiseCoinControl = new ZParadiseCoinControlDialog(this);
    zParadiseCoinControl->setModel(walletModel);
    zParadiseCoinControl->exec();
}

void PrivacyDialog::setZParadiseCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzParadiseCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPRD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ParadiseCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPRDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPRD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PRD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPRD</b>";
    ...
    // use mints from zParadiseCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZParadiseCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZParadiseCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zParadiseCoin selector in case it was used
    ZParadiseCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zParadiseCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ParadiseCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zParadiseCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPRD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPRD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPRD "));
    ui->labelzPRDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPRDSyncStatus->setVisible(fShow);
}
