...
#include "ui_zParadiseCoincontroldialog.h"
...
std::list<std::string> ZParadiseCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZParadiseCoinControlDialog::listMints;

ZParadiseCoinControlDialog::ZParadiseCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZParadiseCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZParadiseCoinControlDialog::~ZParadiseCoinControlDialog()
{
    delete ui;
}

void ZParadiseCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZParadiseCoinControlDialog::updateList()
{
...
void ZParadiseCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZParadiseCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZParadiseCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZParadiseCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZParadiseCoinControlDialog::GetSelectedMints()
{
...
void ZParadiseCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
