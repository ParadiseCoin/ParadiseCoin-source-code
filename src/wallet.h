...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 PRD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPRD send
enum ZerocoinSpendStatus {
    ZPRD_SPEND_OKAY = 0,                            // No error
    ZPRD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPRD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPRD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPRD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPRD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPRD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPRD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPRD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPRD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPRD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPRD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPRD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPRD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPRD_SPENT_USED_ZPRD = 14                       // Coin has already been spend
};
...
public:
    ...
    void ZParadiseCoinBackupWallet();
    ...
    */
    ...
    void setZParadiseCoinAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000 XVG output and keys which can be used for the Masternode
