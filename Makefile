...
pkgdatadir = $(datadir)/ParadiseCoin
pkgincludedir = $(includedir)/ParadiseCoin
pkglibdir = $(libdir)/ParadiseCoin
pkglibexecdir = $(libexecdir)/ParadiseCoin
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/ParadiseCoin-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/ParadiseCoin-config.h
...
ACLOCAL = ${SHELL} /root/zParadiseCoin/build-aux/missing aclocal-1.14
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/zParadiseCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/zParadiseCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/zParadiseCoin/build-aux/missing automake-1.14
AWK = mawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = ParadiseCoin-cli
BITCOIN_DAEMON_NAME = ParadiseCoind
BITCOIN_GUI_NAME = ParadiseCoin-qt
BITCOIN_TX_NAME = ParadiseCoin-tx
...
MAINT = 
MAKEINFO = ${SHELL} /root/zParadiseCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = ParadiseCoin
PACKAGE_BUGREPORT = www.ParadiseCoin.org
PACKAGE_NAME = ParadiseCoin Core
PACKAGE_STRING = ParadiseCoin Core 1.0.0
PACKAGE_TARNAME = ParadiseCoin
PACKAGE_URL = 
...
ZMQ_CFLAGS =  
ZMQ_LIBS = -lzmq  
abs_builddir = /root/zParadiseCoin
abs_srcdir = /root/zParadiseCoin
abs_top_builddir = /root/zParadiseCoin
abs_top_srcdir = /root/zParadiseCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/zParadiseCoin/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_ParadiseCoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_ParadiseCoin_coverage.info test_ParadiseCoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/PRD-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/ParadiseCoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/ParadiseCoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/ParadiseCoin-config.h
$(top_srcdir)/src/config/ParadiseCoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/ParadiseCoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/PRD-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/PRD-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/PRD-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_ParadiseCoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_ParadiseCoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_ParadiseCoin_filtered.info: test_ParadiseCoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_ParadiseCoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_ParadiseCoin_coverage.info: baseline_filtered_combined.info test_ParadiseCoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_ParadiseCoin_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_ParadiseCoin_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_ParadiseCoin_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_ParadiseCoin.coverage/.dirstamp:  test_ParadiseCoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_ParadiseCoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_ParadiseCoin.coverage/ total.coverage/ $(OSX_APP)
